...
// TODO: Naming.
function split<X extends { [key: string]: unknown }>(
  tags: string[],
...
function windowParam(): ParDoLookupParam<Window> {
  return new ParDoLookupParam<Window>("window");
}

function timestampParam(): ParDoLookupParam<Instant> {
  return new ParDoLookupParam<Instant>("timestamp");
}

function paneInfoParam(): ParDoLookupParam<PaneInfo> {
 ...
 */
class Metric<T> extends ParDoUpdateParam<T> {
  constructor(
...
function distribution(name: string): Metric<number> {
